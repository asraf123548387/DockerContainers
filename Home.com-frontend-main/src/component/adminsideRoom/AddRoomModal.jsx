...
const AddRoomModal = ({ isOpen, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(room);
      const response = await axios.post('http://localhost:8080/admin/adminAddRoom', room, {
        headers: {
  ...
  useEffect(() => {
    const fetchHotels = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/admin/adminHotelListInAddRoom/${userId}`, { headers });
